...
range: f511f35ec104bf26ad906772a7545561cefe9d5f..1d5d45163a2e94c9a23e9ea697412d8b5c18c68f
kinds: web-ui
date: 2026-08-20
---
...
> Commit shas in this trail were rewritten on 2026-08-20 to remove an author email that leaked from a mis-set git identity. The tree hash is unchanged — 4350d1e — so every judgement here still refers to the same content under a new sha.

Judged at `1d5d451`. One later commit, `dd5fad5`, adds a single documentation line to `CLAUDE.md` recording that ruleset `21038975` does not revert with the code — the hazard this report names under Rollback. It changes no code and no gate outcome.

Range `f511f35..1d5d451`, 59 commits, 40 files. The base resolved as the branch
point: no tag exists in this repository (`git tag -l` is empty), and
`git merge-base master governance-ci-four-gates` is `f511f35`, which is also
`origin/master`'s tip.
...
- **Nothing merged but unreleased.** `origin/master` is `f511f35`, and the last
  `deploy.yml` run — 2026-08-14, `headSha f511f35` — concluded `success`. What
...
| W-5 the first load is not absurd | met | — | Built at both ends against the same `node_modules`, via a detached worktree at the base. Base `f511f35`: `index-DLfuNgYp.js  170.49 kB │ gzip: 61.88 kB`, CSS `14.48 kB`. Head `1d5d451`: `index-B9EVaLBo.js  170.12 kB │ gzip: 61.76 kB`, CSS `14.51 kB`. The entry chunk **shrank** by 0.37 kB — the four deleted keys across four locales. No new dependency entered the bundle. |
...
- **U-2 — most of this range was never reviewed** *(bucket A)*. The original
  governance work was reviewed at `2b9f9c9` and every one of its 17 findings was
