...
- **Assert the payload round-trips through whichever reader is nearest**: a reader that was ever adapted to accept our own output cannot testify about it. This repository had one of each. `parseVmess` was never adapted and rejects the old shape outright — a gate built on it would have caught this. `dedup.js`'s `vmessConfig`, the reader that actually sat in the pipeline, had grown a second `decodeURIComponent` fallback so dedup kept working against our output, and a gate built on *it* would have passed. Nothing in either signature says which is which, and the safe one is only safe until someone has to make it tolerate a broken writer. Hence a decode path that cannot be adapted, because it is written from the published grammar and imports nothing.
